...
class Logger extends Transform {
  ...
  _addDefaultMeta(msg) {
    if (this.defaultMeta) {
      ...
      // Ensure that we remove all properties from the msgClone that are not writeable in the base object to prevent TypeErrors
      const descriptors = Object.getOwnPropertyDescriptors(msg);
      const nonWritableProperties = Object.keys(descriptors).filter((property) => descriptors[property].writable === false);
      nonWritableProperties.forEach((property) => delete msgClone[property]);
